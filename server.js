...
app.get('/health', (req, res) => {
    res.json({ status: 'ok', keys: keys.size, tokens: oneTimeTokens.size });
});

// Validate key (for GG script)
...
// Request new key token (for Vercel frontend)
app.post('/request-new-key-token', (req, res) => {
    const { deviceId } = req.body;
    const token = crypto.randomBytes(32).toString('hex');
    oneTimeTokens.set(token, {
        deviceId: deviceId || 'web_user',
        expiresAt: Date.now() + (5 * 60 * 1000)
    });
    res.json({ token: token });
});

// Exchange token for key (for Vercel frontend)
app.get('/key', (req, res) => {
